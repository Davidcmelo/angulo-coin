...
export function What(){
    return(
        <Container id="what" maxWidth={false} style={{height:'70%', display:'flex', paddingTop:'30px',textAlign:'center', boxShadow: 'inset 0 0 100px rgba(0, 0, 0, 0.9), 0 0 100px rgba(0, 0, 0, 0.9)'}}>
            <Container>
                <Typography variant='h3' textTransform={'uppercase'} padding={5} >
                    Oque fazemos
                </Typography>
                ...
                <Box display={'flex'} justifyContent={'center'} >
                    <Image src={italia} alt="Logo angulo" style={{animation: 'pulse 5s ease-in-out infinite', width:'120px',}}/>
                    <Image src={brasil} alt="Logo angulo" style={{animation: 'pulse 5s ease-in-out infinite', width:'120px'}}/>
                    <Image src={argentina} alt="Logo angulo" style={{animation: 'pulse 5s ease-in-out infinite', width:'120px'}}/>
                </Box>
            </Container>
            <Container style={{display:'flex', alignItems:'center' }}>
                <Image src={trofeu} alt="Logo angulo" style={{padding:'20px',animation:' swing 5s infinite' }}/>
                <Typography fontSize={'1.5rem'} fontStyle={'italic'} display={'flex'} justifyContent={'center'} alignItems={'center'} flexDirection={'column'}>
                    A partir de dezembro deste ano: Copa Mundial que será realizada em Madrid.
                    <Image src={espanha} alt="Logo angulo" style={{animation: 'pulse 5s ease-in-out infinite', width:'200px'}}/>
                </Typography>
            </Container>
        </Container>
    )
